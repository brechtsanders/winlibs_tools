...
#define PROGRAM_NAME    "wl-query"
#define PROGRAM_DESC    "Command line utility to query installed winlibs packages"
...
int complete_package_name (char *argv[], char* env[], const miniargv_definition* argdef, const miniargv_definition* envdef, const miniargv_definition* currentarg, const char* arg, int argparampos, void* callbackdata)
{
  return 0;
}


int miniargv_show_error (const miniargv_definition* argdef, const char* value, void* callbackdata)
{
  fprintf(stderr, "Error processing argument or environment variable: ");
  if (!argdef || (!argdef->shortarg && !argdef->longarg)) {
    fprintf(stderr, "%s", value);
  } else {
    if (argdef->shortarg) {
      fprintf(stderr, "-%c", argdef->shortarg);
      if (argdef->longarg) {
        fprintf(stderr, " / ");
      }
    }
    if (argdef->longarg) {
      fprintf(stderr, "--%s", argdef->longarg);
    }
  }
  fprintf(stderr, "\n");
  return -1;
...
/*! \brief predefined callback function to parse a configuration file according to the definitions specified in \b userdata
 * \param  argdef                definition of command line argument, or NULL for standalone value argument
 * \param  value                 path of configuration file
 * \param  callbackdata          user data passed to \a miniargv_process_cfgfile
 ...
 */
DLL_EXPORT_MINIARGV int miniargv_cb_process_cfgfile (const miniargv_definition* argdef, const char* value, void* callbackdata);
...
DLL_EXPORT_MINIARGV int miniargv_cb_process_cfgfile (const miniargv_definition* argdef, const char* value, void* callbackdata)
{
  ...
  if (stat(value, &fileinfo) == -1 || !S_ISREG(fileinfo.st_mode)) {
    fprintf(stderr, "Configuration file not found: %s\n", value);
    return -1;
  }
  return miniargv_process_cfgfile(value, (const miniargv_definition*)argdef->userdata, callbackdata);
}
...
*/
  ...
  const miniargv_definition argdef[] = {
    ...
    {'c', "config", "FILE", miniargv_cb_process_cfgfile, argdef, "config file (optional, overrides $WL_CONFIG)", NULL},
    {'i', "installpath", "PATH", miniargv_cb_strdup, &dstdir, "installation path (overrides $MINGWPREFIX)", NULL},
    {'d', "dummy", "VALUE", miniargv_cb_noop, NULL, "dummy parameter for testing", NULL},
/*
...
*/
  ...
  const miniargv_definition envdef[] = {
    {0, "WL_CONFIG", "PATH", miniargv_cb_process_cfgfile, argdef, "config file (optional)", NULL},
    {0, "MINGWPREFIX", "PATH", miniargv_cb_strdup, &dstdir, "installation path", NULL},
    MINIARGV_DEFINITION_END
  };
  //check if we are being called for bash completion (Tab key on the command line, configured via: complete -C)
  if (miniargv_completion(argv, envp, argdef, envdef, "--completion", NULL))
    return 0;

//if (miniargv_process_cfgfile(cfgfile, argdef, NULL);
  //parse environment and command line flags
/*
  if (miniargv_process(argv, envp, argdef, envdef, NULL, &searchtype) != 0)
*/
  if (miniargv_process(argv, envp, argdef, envdef, miniargv_show_error, NULL) != 0)
    return 1;
